...
def build_arg_parser():
    ...
    if PORT_DEFAULT:
        ...
        portkwargs = {"required": True, "help": porthelp + " Required."}
    ...
    parser.add_argument(
        ...
        + "default %d (%d minutes)"
        % (DIE_AFTER_SECONDS_DEFAULT, DIE_AFTER_SECONDS_DEFAULT / 60),
    )
    ...
    parser.add_argument(
        ...
        help="When server is asked to shutdown, give it this many seconds to shutdown cleanly. Default: {}".format(
            SHUTDOWN_GRACE_PERIOD_DEFAULT
        ),
    )
    ...
    parser.add_argument(
        ...
        help="Logging level. ERROR, WARN, INFO, DEBUG. Default: {}".format(
            LOG_LEVEL_DEFAULT
        ),
    )
...
class WormProcess(object):
    def __init__(self, executable_content, exec_args=[], popen_kwargs={}):
        ...
        self.execfile = tempfile.NamedTemporaryFile(
            dir="/dev/shm", prefix="inf3203_worm_assignment_", delete=False
        )
        ...
        logger.info(
            "Started subprocess. PID %d. Command: %s. Popen args: %s.",
            self.popen.pid,
            self.cmd,
            self.popen_kwargs,
        )
    ...
    def __str__(self):
        return "WormProcess{{PID={}, cmd={}, popen_kwargs={}}}".format(
            self.popen.pid, self.cmd, self.popen_kwargs
        )
        ...
        with self.lock:
            ...
            for proc in self.processes:
                ...
                if poll != None:
                    ...
                    logger.info(
                        "Segment has stopped with exit code %s: %s",
                        poll,
                        proc.execfile.name,
                    )
                    proc.cleanup()
...
def run_http_server(args):
    ...
    def shutdown_server_with_grace_period(thread):
        ...
        if thread.is_alive():
            logger.error(
                "Server thread is still alive after %.3f-second grace period. Trying to exit anyway.",
